...
window._resolveMetaToken = async function () {
  ...
  // --- Quyết định token nào sẽ được verify (theo thứ tự ưu tiên của initAccountSelector) ---
  let tokenToVerify = null;
  let tokenSource = "";

  // 1. Kiểm tra trong dom_allowed_accounts trước (Multi-token)
  if (Array.isArray(allowedAccounts) && allowedAccounts.length > 0) {
    const firstGroup = allowedAccounts[0];
    // Định dạng cấu hình mới: có trường 'token'
    if (firstGroup && typeof firstGroup === 'object' && firstGroup.token) {
      // Tìm nhóm chứa account_id đang chọn
      for (const g of allowedAccounts) {
        const found = (g.accounts || []).find(a => a.id === savedId || a.id.replace('act_', '') === savedId);
        if (found) {
          tokenToVerify = g.token;
          tokenSource = "dom_allowed_accounts";
          break;
        }
      }
      // Nếu không thấy, lấy token nhóm đầu tiên có account làm fallback
      if (!tokenToVerify) {
        for (const g of allowedAccounts) {
          if (g.accounts && g.accounts.length > 0) {
            tokenToVerify = g.token;
            tokenSource = "dom_allowed_accounts_fallback";
            break;
          }
        }
      }
  ...
  // 2. Nếu không có trong dom_allowed_accounts, dùng settingsToken (meta_access_token)
  if (!tokenToVerify && settingsToken) {
    tokenToVerify = settingsToken;
    tokenSource = "meta_access_token";
  }

  // 3. Cuối cùng mới thử dùng META_TOKEN_STATIC cứng trong code
  if (!tokenToVerify && META_TOKEN_STATIC && META_TOKEN_STATIC.length > 20) {
    tokenToVerify = META_TOKEN_STATIC;
    tokenSource = "token.js (static)";
  }

  // --- Tiến hành Verify token đã tìm thấy ---
  if (tokenToVerify) {
    const res = await _verifyToken(tokenToVerify);
    if (res.ok) {
      console.log(`[token] ✅ Dùng token hợp lệ từ ${tokenSource}`);
      _applyToken(tokenToVerify);
      try { sessionStorage.setItem(_TOKEN_VERIFIED_KEY, JSON.stringify({ token: tokenToVerify, ts: Date.now() })); } catch (_) { }
      return;
    }
    
    // Nếu token bị thu hồi do đổi mật khẩu
    if (res.reason === 'password_changed') {
      console.warn(`[token] 🔴 Token từ ${tokenSource} bị thu hồi do đổi mật khẩu Facebook (subcode 460).`);
      window._tokenFailReason = 'password_changed';
    } else {
      console.warn(`[token] ⚠️ Token từ ${tokenSource} đã hết hạn hoặc không hợp lệ.`);
    }
  }
