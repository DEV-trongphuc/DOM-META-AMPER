...
async function _sheetGet() {
    ...
    try {
        const acc = window.ACCOUNT_ID || (typeof ACCOUNT_ID !== 'undefined' ? ACCOUNT_ID : '676599667843841');
        const res = await fetch(`${SETTINGS_SYNC_URL}?sheet=settings&account_id=${acc}&_t=${Date.now()}`, { method: "GET" });
...
function _applyToGlobal(key, value) {
    switch (key) {
        ...
        case "dom_summary_metrics":
            ...
            break;
        case "dom_allowed_accounts":
            if (Array.isArray(value)) window.ALLOWED_ACCOUNTS = value;
            break;
    }
}
...
/**
 * Save allowed accounts (multi-token workspaces) to DB silently in background.
 */
window.saveAllowedAccountsSync = function (groups) {
    return _sheetPost({ key: "dom_allowed_accounts", value: groups });
};
